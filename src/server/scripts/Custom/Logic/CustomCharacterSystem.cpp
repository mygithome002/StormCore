...
void CustomCharacterSystem::completeAddPlayTimeReward(int playtime, Player* player, int money)
{
	...
	if (!result) {
		...
		insertNewPlayerPlayTimeReward(playtime, player->GetSession()->GetPlayerName(), player->GetGUID().GetCounter());
		if (playtime == 100) {
...
int CustomCharacterSystem::getGUID(int accountid)
{
	PreparedStatement * stmt = CharacterDatabase.GetPreparedStatement(CHAR_SEL_CHARACTER_GUID);
	stmt->setInt32(0, accountid);
	PreparedQueryResult result = CharacterDatabase.Query(stmt);
